                        ...
                        <tbody>
                            <tr v-for="row in filterData" :key="row.id">
                                ...
                                <td class="text-center">
                                    <router-link :to="{ name: 'management-rolesetting', params: { id : row.id }}" class="btn btn-sm btn-warning"><feather-icon icon="SettingsIcon"></feather-icon> Setting</router-link>
                                    <button @click="edit(row.id)" class="btn btn-sm btn-success"><feather-icon icon="Edit3Icon"></feather-icon> Edit</button>
                                    <button class="btn btn-sm btn-danger" @click="hapus(row.id)"><feather-icon icon="DeleteIcon"></feather-icon> Delete</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
//            datatablecomponent
        ...
        methods : {
            ...
            slugify: function () {
                let slugInput = this.dataForm.name.toLowerCase()
                slugInput = slugInput.replace(/\s*$/g, '');
                slugInput = slugInput.replace(/\s+/g, '-');

                this.dataForm.slug = slugInput
            },
            getData: function() {
             ...
             addModul(){
                ...
                if(this.editId == 0){
                    axios.post(modul, this.dataForm).then(response =>(
                        this.$swal(this.$func.notifAlert('success','Berhasil','Data Role Berhasil Disimpan'))
                    ))
                    ...
                    axios.put(modul+'/'+this.editId, this.dataForm).then(response =>(
                        ...
                        this.$swal(this.$func.notifAlert('success','Berhasil','Data Role Berhasil Diperbaharui'))
                    ))
                    .catch(err=> console.log(err))
                    .finally(() =>
                        this.getData()
